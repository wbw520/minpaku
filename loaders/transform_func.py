...
class Aug(object):
    ...
    def __call__(self, image, ratio):
        if self.aug:
            ImgAug = ImageAugment(ratio)   # ImageAugment class will augment the img and label at same time
...
class ToTensor(object):
    ...
    def __call__(self, image, ratio, color=True):
        if image.ndim == 2:
            image = image[:, :, None]
